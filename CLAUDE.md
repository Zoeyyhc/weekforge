...
- `src/weekforge/api/` — FastAPI app (`app.py`/`server.py`), routes (`routes.py` authenticated debate + SSE, `auth_routes.py` signup/login/me/preferences + `get_current_user`, `ics_routes.py` export), `sse.py`, `sessions.py`.
- `src/weekforge/auth/` — local account support: `store.py` (`UserStore`, SQLite `users` table in `WEEKFORGE_DB_PATH`, bcrypt password hashes, saved preferences) and `tokens.py` (HS256 JWT helpers).
...
- **Auth is local only:** accounts are email/password/display-name records in the local SQLite DB. No OAuth, no third-party auth, and no calendar write access. Debate endpoints require auth; `/debate/{thread_id}/stream` accepts the session token for EventSource.
...
| `WEEKFORGE_AUTH_SECRET` | Required HS256 signing key for 7-day local-account session JWTs |
